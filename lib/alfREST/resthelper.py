...
class RESTHelper(object):
    ...
    def initiateBulkImport(self, sourceDirectory, targetPath):
        ...
        method, url = parseComm(INITIATEBULKIMPORT[1])
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/plain"}
        res = self.request(method, url, headers=headers, params=params)
        if res is not None:
            logger.info('Import Initiate')
            return True
        return False
        ...
        if method in ("GET", "DELETE"):
            ...
            if headers is None:
                ...
                if ( ctype == "application/x-www-form-urlencoded" ):
                    http_conn.request(method, url, urllib.urlencode(params), headers)
                else:
